...
class UIComponents {
    ...
    public static JPanel backgroundImagePanel(String imgPath) {
        ...
        try {
            if (!imgPath.isEmpty()) {
                ImageIcon icon = new ImageIcon(UIComponents.class.getResource(imgPath));
        ...
        return new JPanel(new BorderLayout()) {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (finalBgImage != null) {
                    ...
                    int imgW = finalBgImage.getWidth(null);
                    int imgH = finalBgImage.getHeight(null);
                    int panelW = getWidth();
                    ...
                    g2d.drawImage(finalBgImage, drawX, drawY, drawW, drawH, this);
                } else {
                    g.setColor(WellTrackConstants.BG_WHITE);
...
class PanelFactory {
    ...
    private Runnable updateSummaryCallback;
    
    public PanelFactory(JLabel summaryLabel, Runnable updateSummaryCallback) {
    ...
    public JPanel createHomePanel() {
        ...
        JPanel topSection = new JPanel(new BorderLayout()) {
            ...
            {
                try {
                    ImageIcon icon = new ImageIcon(getClass().getResource("/images/home.jpg"));
                    bgImage = icon.getImage();
                } catch (Exception e) {}
            }
        ...
        // Stats and footer section with solid black background
    ...
    public JPanel createNotesPanel() {
        ...
        JPanel bgImageSection = UIComponents.backgroundImagePanel("/images/insights.jpg");
        bgImageSection.setPreferredSize(new Dimension(1600, 900));
        bgImageSection.setMinimumSize(new Dimension(1600, 900));
        bgImageSection.setMaximumSize(new Dimension(5000, 900));
        bgImageSection.setLayout(new BorderLayout());
    ...
    private void clearFields(JComponent[] fields) {
        for (JComponent f : fields) {
            ...
            else if (f instanceof JTextArea) ((JTextArea) f).setText("");
        }
    }
...
public class WellTrackUI {
    private int totalWorkouts = 0, totalCups = 0, totalSleep = 0, totalGoals = 0;
    private String lastFood = "";
    private String lastFoodDate = "";
    private String lastWorkoutDate = "";
    private String lastWorkoutType = "";
    private String lastSleepDate = "";
    ...
    public WellTrackUI() {
        ...
        panelFactory = new PanelFactory(summaryLabel, this::updateSummary);

        // initialize database manager early so forms can persist data
        ...
        tabs.setBackground(WellTrackConstants.BG_BLACK);
        ...
        tabs.addTab("Sleep", panelFactory.createFormPanel("Sleep Tracker", "Track your rest",
            ...
            fields -> {
                ...
                lastSleepDate = date;
                totalSleep += Integer.parseInt(hours);
                updateSummary();
        ...
        tabs.addTab("Nutrition", panelFactory.createFormPanel("Nutrition Tracker", "Monitor your meals",
            ...
            fields -> {
                ...
                lastFood = foodName + " (" + mealType + ")";
                lastFoodDate = date;
                updateSummary();
                showMsg("Meal logged: " + foodName);
            }));
            
        tabs.addTab("Goals", panelFactory.createFormPanel("Goals", "Achieve your targets",
            new String[]{"Goal Name:", "Goal Type:", "Target Value:", "Current Value:", "Start Date:", "End Date:", "Status:"},
            new String[]{"text", "combo:Weight,Strength,Endurance,Flexibility", "text", "text", "date", "date", "combo:Active,Completed,Paused"},
            fields -> {
                String goalName = ((JTextField)fields[0]).getText();
                String goalType = (String)((JComboBox<?>)fields[1]).getSelectedItem();
                String targetValue = ((JTextField)fields[2]).getText();
                String currentValue = ((JTextField)fields[3]).getText();
                String startDate = ((JTextField)fields[4]).getText();
                String endDate = ((JTextField)fields[5]).getText();
                String status = (String)((JComboBox<?>)fields[6]).getSelectedItem();
                
                // persist goal data
                dbManager.saveUserGoal(goalName, goalType, targetValue, currentValue, startDate, endDate, status);

                lastGoalName = goalName;
                lastGoalDate = startDate;
                totalGoals++;
                updateSummary();
                showMsg("Goal added: " + goalName);
            }));
    ...
    private String summaryHTML() {
        return "<html><div style='line-height:1.8;'>" +
            ...
            "<p style='font-size:13px;color:#1a1a1a;margin:6px 0;'><strong style='color:#7C3AED;'>Workouts:</strong> " + totalWorkouts + " logged" + 
            (lastWorkoutDate.isEmpty() ? "" : " <span style='color:#6B7280;font-size:12px;'>(Latest: " + lastWorkoutType + " - " + lastWorkoutDate + ")</span>") + "</p>" +
            "<p style='font-size:13px;color:#1a1a1a;margin:6px 0;'><strong style='color:#3B82F6;'>Hydration:</strong> " + totalCups + " cups logged</p>" +
            "<p style='font-size:13px;color:#1a1a1a;margin:6px 0;'><strong style='color:#EC4899;'>Sleep:</strong> " + totalSleep + " total hours" +
            (lastSleepDate.isEmpty() ? "" : " <span style='color:#6B7280;font-size:12px;'>(Latest entry: " + lastSleepDate + ")</span>") + "</p>" +
            "<p style='font-size:13px;color:#1a1a1a;margin:6px 0;'><strong style='color:#7C3AED;'>Goals:</strong> " + totalGoals + " created" + 
            (lastGoalDate.isEmpty() ? "" : " <span style='color:#6B7280;font-size:12px;'>(\"" + lastGoalName + "\" started " + lastGoalDate + ")</span>") + "</p>" +
            "<p style='font-size:13px;color:#1a1a1a;margin:6px 0;'><strong style='color:#10B981;'>Nutrition:</strong> " + (lastFood.isEmpty() ? "No meals logged yet" : lastFood) + 
            (lastFoodDate.isEmpty() ? "" : " <span style='color:#6B7280;font-size:12px;'>on " + lastFoodDate + "</span>") + "</p>" +
            "</div></html>";
    }
